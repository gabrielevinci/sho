...
// Definiamo il tipo per un singolo link, includendo i nuovi campi opzionali
          ...
          {links.map((link) => {
            ...
            return (
              <tr key={link.short_code}>
                {/* Colonna Titolo e URL Originale */}
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-semibold text-gray-900 truncate max-w-xs">
                    {link.title || 'Nessun Titolo'}
                  </div>
                  <a href={link.original_url} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-500 hover:text-gray-700 truncate block max-w-xs">
                    {link.original_url}
                  </a>
                </td>

                {/* Colonna Link Shortato */}
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {new Date(link.created_at).toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                </td>

                {/* Colonna Azioni (Copia) */}
