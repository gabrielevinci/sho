    ...
    <div className="flex flex-col min-h-screen bg-gray-50 py-8">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {/* Barra di progresso comparativa */}
          <div className="mt-6 pt-6 border-t border-gray-100">
            <div className="text-sm font-medium text-gray-700 mb-3">Confronto periodi</div>
            <div className="space-y-3">
              {/* Barra oggi vs settimana */}
              <div className="flex items-center space-x-4">
                <span className="text-xs text-gray-600 w-20">Oggi/7gg:</span>
                <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-blue-500 transition-all duration-500"
                    style={{ 
                      width: `${initialData.clickAnalytics.clicks_this_week > 0 
                        ? Math.min((initialData.clickAnalytics.clicks_today / initialData.clickAnalytics.clicks_this_week) * 100, 100) 
                        : 0}%` 
                    }}
                  />
                </div>
                <span className="text-xs text-gray-600 w-12">
                  {initialData.clickAnalytics.clicks_this_week > 0 
                    ? `${Math.round((initialData.clickAnalytics.clicks_today / initialData.clickAnalytics.clicks_this_week) * 100)}%`
                    : '0%'
                  }
                </span>
              </div>
              
              {/* Barra settimana vs mese */}
              <div className="flex items-center space-x-4">
                <span className="text-xs text-gray-600 w-20">7gg/30gg:</span>
                <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-green-500 transition-all duration-500"
                    style={{ 
                      width: `${initialData.clickAnalytics.clicks_this_month > 0 
                        ? Math.min((initialData.clickAnalytics.clicks_this_week / initialData.clickAnalytics.clicks_this_month) * 100, 100) 
                        : 0}%` 
                    }}
                  />
                </div>
                <span className="text-xs text-gray-600 w-12">
                  {initialData.clickAnalytics.clicks_this_month > 0 
                    ? `${Math.round((initialData.clickAnalytics.clicks_this_week / initialData.clickAnalytics.clicks_this_month) * 100)}%`
                    : '0%'
                  }
                </span>
              </div>
