...
async function finalTest() {
  try {
    console.log(`Test finale delle query SQL per shortCode: ${shortCode}`);
    ...
    // Test di entrambe le query insieme
    const [cityResult, browserResult] = await Promise.all([
      // Query città finale
      sql`
    ...
    // Calcola i totali per entrambe le query
    let cityTotal = 0;
    let browserTotal = 0;
    
    console.log('\n=== RISULTATI QUERY CITTÀ ===');
    console.log('Primi 15 risultati:');
    cityResult.rows.slice(0, 15).forEach(row => {
      console.log(`${row.city}: ${row.count} click`);
      cityTotal += parseInt(row.count);
    });
    
    if (cityResult.rows.length > 15) {
      let remainingTotal = 0;
      cityResult.rows.slice(15).forEach(row => {
        remainingTotal += parseInt(row.count);
      });
      console.log(`... e altri ${cityResult.rows.length - 15} città con totale di ${remainingTotal} click`);
      cityTotal += remainingTotal;
    }
    console.log(`\nTotale click città: ${cityTotal}`);
    
    console.log('\n=== RISULTATI QUERY BROWSER ===');
    browserResult.rows.forEach(row => {
      console.log(`${row.browser}: ${row.count} click`);
      browserTotal += parseInt(row.count);
    });
    console.log(`\nTotale click browser: ${browserTotal}`);
    
    // Verifica finale
    console.log('\n=== VERIFICA FINALE ===');
    console.log(`Totale click effettivi: ${totalClicks}`);
    console.log(`Totale click città: ${cityTotal}`);
    console.log(`Totale click browser: ${browserTotal}`);
    
    if (cityTotal === totalClicks && browserTotal === totalClicks) {
      console.log('✅ SUCCESSO! Entrambe le query contano correttamente tutti i click.');
    } else {
      if (cityTotal !== totalClicks) {
        console.log(`❌ ERRORE CITTÀ: Mancano ${totalClicks - cityTotal} click nella query città.`);
      }
      if (browserTotal !== totalClicks) {
        console.log(`❌ ERRORE BROWSER: Mancano ${totalClicks - browserTotal} click nella query browser.`);
      }
    }
